...
export class RtsWorld {
  ...
  update(deltaTime: number): void {
    ...
    if (this.spawnTimer >= Math.max(14, 34 - this.state.day * 2) && this.unitCount < MAX_ACTIVE_UNITS_SOFT) {
      this.spawnTimer = 0;
      this.spawnRaid();
    }
  ...
  commandSelectedAttack(targetId: string, lockedTarget = false): void {
    this.state.selectedUnitIds.forEach((id) => {
      const unit = this.state.units[id];
      if (unit) unit.order = { type: 'attack', targetId, lockedTarget };
    });
  }
  ...
  private findSettlementAnchor(preferredTileX: number, preferredTileY: number, footprintW: number, footprintH: number): { x: number; y: number } | null {
    const tryPoint = (tx: number, ty: number) => {
      if (tx < 2 || ty < 2 || tx + footprintW >= MAP_WIDTH - 2 || ty + footprintH >= MAP_HEIGHT - 2) return null;
  ...
  private uvToTile(u: number, v: number): { x: number; y: number } {
    return {
      x: Math.floor(u * (MAP_WIDTH - 8)) + 4,
      y: Math.floor(v * (MAP_HEIGHT - 8)) + 4,
    };
  }
  ...
  private spawnUnitSafe(factionId: string, role: UnitRole, nearTileX: number, nearTileY: number, slot: number): Unit {
  ...
  private seedSettlements(): void {
    ...
    this.log('边境城邦建立于欧陆平原。北境守望是盟友，赤潮军阀是敌人。');
  }
  ...
  private updateAttackOrder(unit: Unit, deltaTime: number): void {
    ...
    if (unit.attackCooldown <= 0) {
      ...
      if ('role' in target) {
        this.forceCounterAttack(target, unit);
      }
  ...
  private findNearestResourceTile(unit: Unit, resource: ResourceType): Tile | null {
    ...
    const collectInRadius = (radiusTiles: number) => {
      ...
      const endX = Math.min(MAP_WIDTH - 1, centerTileX + radiusTiles);
      const startY = Math.max(0, centerTileY - radiusTiles);
      const endY = Math.min(MAP_HEIGHT - 1, centerTileY + radiusTiles);
